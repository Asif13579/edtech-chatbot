...
SYSTEM_PROMPT="""You are EduBot, a friendly and knowleadge customer support assistant for learnLearnSphere — an online EdTech platform offering courses in Programming, Data Science, Design, Business, and Personal Development.
...
async def chat(request: ChatRequest):
    ...
    try:
        ...
        # completion=client.chat.completions.create(model='llama-3.3-70b-versatile',messages=messages,max_tokens=1024,temperature=0.6,)
        # reply=completion.choices[0].message.content
        # tokens=completion.usage.total_tokens
        # return ChatResponse(reply=reply,tokens_used=tokens)

    except Exception as e:
        raise HTTPException(status_code=500,detail=f"Gemini API errors: {str(e)}")
